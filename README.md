...
[Digital Ocean Deploy]: https://cloud.digitalocean.com/apps/new?repo=https://github.com/Toprent-app/mixpanel-tracking-proxy/tree/master
...
[Render Deploy]: https://render.com/deploy?repo=https://github.com/Toprent-app/mixpanel-tracking-proxy
...
[Heroku Deploy]: https://heroku.com/deploy/?template=https://github.com/Toprent-app/mixpanel-tracking-proxy
...
### Option 2: Docker Image
   ...
   1. Clone the repo 
   `git clone https://github.com/Toprent-app/mixpanel-tracking-proxy`
...
```
This is same response you would get from visiting https://api-eu.mixpanel.com/ (which means your proxy is working as expected).
...
If you already have servers running nginx, you can copy and paste the locations from the [nginx.conf](https://github.com/Toprent-app/mixpanel-tracking-proxy/blob/master/nginx.conf) file in this repo and adjust the locations to match your preference.

### Additional Configuration
In case you see events tracked to your project, but geo-location properties are not being added correctly, there's likely a configuration issue with passing the IP address through the headers. Within the [nginx configuration]([url](https://github.com/Toprent-app/mixpanel-tracking-proxy/blob/2c08e999d4b38aa943fad55884bcfe0ef72bb681/nginx.conf#L31)), by default, we leverage `$http_x_forwarded_for` and pass it through the `X-Real-IP` header to forward the IP from the original client; in some providers, this is not available and you will want to review which header/variable provides the IP address from the client. As an example, in Cloudflare, you need to pass `$realip_remote_addr` instead. You'll want to review the option that applies to your cloud provider.
